...
static size_t find_bytes(BYTE* ps, size_t ls, BYTE* pb, size_t lb, int mode, BYTE (*cmp)(BYTE));
 ...
 * @return Position of found string or -1 if not there.
*/
size_t find_bytes(BYTE* ps, size_t ls, BYTE* pb, size_t lb, int mode, BYTE (*cmp)(BYTE))
{
	size_t start, inc, end, i;
	if (mode == 1)
	{
		start = 0;
		inc = 1;
